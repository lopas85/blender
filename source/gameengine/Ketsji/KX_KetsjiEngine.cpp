...
KX_KetsjiEngine::RenderData KX_KetsjiEngine::GetRenderData()
{
	...
	renderData.m_stereoMode = stereomode;
	renderData.m_renderPerEye = renderpereye;
	renderData.m_frameSettings = m_scenes->GetFront()->GetFramingType();
	...
	for (unsigned short frame = 0; frame < numframes; ++frame) {
		const FrameRenderData frameData = GetFrameRenderData(stereomode, usestereo, renderpereye, frame);
...
// 	scene->RunDrawingCallbacks(KX_Scene::PRE_DRAW, rendercam); // TODO
	...
	scene->RenderBuckets(objects, m_rasterizer->GetDrawingMode(), cameraFrameData.m_camTrans, m_rasterizer, offScreen);
